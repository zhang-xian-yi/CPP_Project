...
namespace WindowsNS
{
	...
	public:
		...
		~VertexBufferLayout() {}
		//返回布局元素集合
		inline const std::vector<WindowsNS::VLayoutElement>& GetElements()const
		{
			return m_Elements;
		}
		//返回步长
		...
		{
			WindowsNS::VLayoutElement obj(GL_FLOAT,count,false);
		...
		{
			WindowsNS::VLayoutElement obj(GL_UNSIGNED_INT, count, false);
			m_Elements.push_back(obj);
		...
		void Push<unsigned char>(unsigned int count)
		{
			...
			WindowsNS::VLayoutElement obj(GL_UNSIGNED_BYTE, count, true);
			m_Elements.push_back(obj);
			m_Stride += count * VLayoutElement::GetSizeOfType(GL_UNSIGNED_BYTE);
		}
	private:
		//布局元素的向量集合
		std::vector<WindowsNS::VLayoutElement> m_Elements;
