	...
	bool InitAndRegisterAllService::InitAndRegisterMdl(MdlCommonNS::EModuleType type, MdlCommonNS::IMdlFactory* factory)
	{
		...
		try
		{
			...
			if (iLogS.has_value())
			{
				...
				prunlog->LogRunMsg(LoggerNS::ELogLevel::E_Info_LV, MdlCommonNS::EnumModuleTypeExtend::GetInstance()->GetMdlCnDesc(type)+"注册成功");
		...
		catch (std::exception exp)
		{
			...
			{
				auto prunlog = iLogS.value()->ConvertType<LoggerNS::IRunLogger*>();
				prunlog->LogRunMsg(LoggerNS::ELogLevel::E_Error_LV, MdlCommonNS::EnumModuleTypeExtend::GetInstance()->GetMdlCnDesc(type) + "注册失败");
